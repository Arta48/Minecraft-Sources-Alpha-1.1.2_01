public class ci implements nm {
   private int a;
   private int b;
   private ga[][] c;
   private cn d;

   public ci(cn var1, int var2, int var3, int var4, int var5, int var6, int var7) {
      this.d = var1;
      this.a = var2 >> 4;
      this.b = var4 >> 4;
      int var8 = var5 >> 4;
      int var9 = var7 >> 4;
      this.c = new ga[var8 - this.a + 1][var9 - this.b + 1];

      for(int var10 = this.a; var10 <= var8; ++var10) {
         for(int var11 = this.b; var11 <= var9; ++var11) {
            this.c[var10 - this.a][var11 - this.b] = var1.b(var10, var11);
         }
      }

   }

   public int a(int var1, int var2, int var3) {
      if (var2 < 0) {
         return 0;
      } else if (var2 >= 128) {
         return 0;
      } else {
         int var4 = (var1 >> 4) - this.a;
         int var5 = (var3 >> 4) - this.b;
         return this.c[var4][var5].a(var1 & 15, var2, var3 & 15);
      }
   }

   public ic b(int var1, int var2, int var3) {
      int var4 = (var1 >> 4) - this.a;
      int var5 = (var3 >> 4) - this.b;
      return this.c[var4][var5].d(var1 & 15, var2, var3 & 15);
   }

   public float c(int var1, int var2, int var3) {
      return cn.i[this.d(var1, var2, var3)];
   }

   public int d(int var1, int var2, int var3) {
      return this.a(var1, var2, var3, true);
   }

   public int a(int var1, int var2, int var3, boolean var4) {
      if (var1 >= -32000000 && var3 >= -32000000 && var1 < 32000000 && var3 <= 32000000) {
         if (var4) {
            int var5 = this.a(var1, var2, var3);
            if (var5 == ly.al.bc || var5 == ly.aB.bc) {
               int var13 = this.a(var1, var2 + 1, var3, false);
               int var7 = this.a(var1 + 1, var2, var3, false);
               int var8 = this.a(var1 - 1, var2, var3, false);
               int var9 = this.a(var1, var2, var3 + 1, false);
               int var10 = this.a(var1, var2, var3 - 1, false);
               if (var7 > var13) {
                  var13 = var7;
               }

               if (var8 > var13) {
                  var13 = var8;
               }

               if (var9 > var13) {
                  var13 = var9;
               }

               if (var10 > var13) {
                  var13 = var10;
               }

               return var13;
            }
         }

         if (var2 < 0) {
            return 0;
         } else if (var2 >= 128) {
            int var12 = 15 - this.d.e;
            if (var12 < 0) {
               var12 = 0;
            }

            return var12;
         } else {
            int var11 = (var1 >> 4) - this.a;
            int var6 = (var3 >> 4) - this.b;
            return this.c[var11][var6].c(var1 & 15, var2, var3 & 15, this.d.e);
         }
      } else {
         return 15;
      }
   }

   public int e(int var1, int var2, int var3) {
      if (var2 < 0) {
         return 0;
      } else if (var2 >= 128) {
         return 0;
      } else {
         int var4 = (var1 >> 4) - this.a;
         int var5 = (var3 >> 4) - this.b;
         return this.c[var4][var5].b(var1 & 15, var2, var3 & 15);
      }
   }

   public gb f(int var1, int var2, int var3) {
      int var4 = this.a(var1, var2, var3);
      return var4 == 0 ? gb.a : ly.n[var4].bn;
   }

   public boolean g(int var1, int var2, int var3) {
      ly var4 = ly.n[this.a(var1, var2, var3)];
      return var4 == null ? false : var4.b();
   }
}
