import java.util.List;
import java.util.Random;

public class al extends ly {
   private js a;

   protected al(int var1, int var2, js var3) {
      super(var1, var2, gb.d);
      this.a = var3;
      this.b(true);
      float var4 = 0.0625F;
      this.a(var4, 0.0F, var4, 1.0F - var4, 0.03125F, 1.0F - var4);
   }

   public int a() {
      return 20;
   }

   public cf d(cn var1, int var2, int var3, int var4) {
      return null;
   }

   public boolean b() {
      return false;
   }

   public boolean c() {
      return false;
   }

   public boolean a(cn var1, int var2, int var3, int var4) {
      return var1.g(var2, var3 - 1, var4);
   }

   public void e(cn var1, int var2, int var3, int var4) {
   }

   public void a(cn var1, int var2, int var3, int var4, int var5) {
      boolean var6 = false;
      if (!var1.g(var2, var3 - 1, var4)) {
         var6 = true;
      }

      if (var6) {
         this.b_(var1, var2, var3, var4, var1.e(var2, var3, var4));
         var1.d(var2, var3, var4, 0);
      }

   }

   public void a(cn var1, int var2, int var3, int var4, Random var5) {
      if (var1.e(var2, var3, var4) != 0) {
         this.h(var1, var2, var3, var4);
      }
   }

   public void b(cn var1, int var2, int var3, int var4, kh var5) {
      if (var1.e(var2, var3, var4) != 1) {
         this.h(var1, var2, var3, var4);
      }
   }

   private void h(cn var1, int var2, int var3, int var4) {
      boolean var5 = var1.e(var2, var3, var4) == 1;
      boolean var6 = false;
      float var7 = 0.125F;
      List var8 = null;
      if (this.a == js.a) {
         var8 = var1.b((kh)null, (cf)cf.b((double)((float)var2 + var7), (double)var3, (double)((float)var4 + var7), (double)((float)(var2 + 1) - var7), (double)var3 + (double)0.25F, (double)((float)(var4 + 1) - var7)));
      }

      if (this.a == js.b) {
         var8 = var1.a(ge.class, cf.b((double)((float)var2 + var7), (double)var3, (double)((float)var4 + var7), (double)((float)(var2 + 1) - var7), (double)var3 + (double)0.25F, (double)((float)(var4 + 1) - var7)));
      }

      if (this.a == js.c) {
         var8 = var1.a(dm.class, cf.b((double)((float)var2 + var7), (double)var3, (double)((float)var4 + var7), (double)((float)(var2 + 1) - var7), (double)var3 + (double)0.25F, (double)((float)(var4 + 1) - var7)));
      }

      if (var8.size() > 0) {
         var6 = true;
      }

      if (var6 && !var5) {
         var1.b(var2, var3, var4, 1);
         var1.g(var2, var3, var4, this.bc);
         var1.g(var2, var3 - 1, var4, this.bc);
         var1.b(var2, var3, var4, var2, var3, var4);
         var1.a((double)var2 + (double)0.5F, (double)var3 + 0.1, (double)var4 + (double)0.5F, "random.click", 0.3F, 0.6F);
      }

      if (!var6 && var5) {
         var1.b(var2, var3, var4, 0);
         var1.g(var2, var3, var4, this.bc);
         var1.g(var2, var3 - 1, var4, this.bc);
         var1.b(var2, var3, var4, var2, var3, var4);
         var1.a((double)var2 + (double)0.5F, (double)var3 + 0.1, (double)var4 + (double)0.5F, "random.click", 0.3F, 0.5F);
      }

      if (var6) {
         var1.h(var2, var3, var4, this.bc);
      }

   }

   public void b(cn var1, int var2, int var3, int var4) {
      int var5 = var1.e(var2, var3, var4);
      if (var5 > 0) {
         var1.g(var2, var3, var4, this.bc);
         var1.g(var2, var3 - 1, var4, this.bc);
      }

      super.b(var1, var2, var3, var4);
   }

   public void a(nm var1, int var2, int var3, int var4) {
      boolean var5 = var1.e(var2, var3, var4) == 1;
      float var6 = 0.0625F;
      if (var5) {
         this.a(var6, 0.0F, var6, 1.0F - var6, 0.03125F, 1.0F - var6);
      } else {
         this.a(var6, 0.0F, var6, 1.0F - var6, 0.0625F, 1.0F - var6);
      }

   }

   public boolean b(nm var1, int var2, int var3, int var4, int var5) {
      return var1.e(var2, var3, var4) > 0;
   }

   public boolean c(cn var1, int var2, int var3, int var4, int var5) {
      if (var1.e(var2, var3, var4) == 0) {
         return false;
      } else {
         return var5 == 1;
      }
   }

   public boolean d() {
      return true;
   }

   public void e() {
      float var1 = 0.5F;
      float var2 = 0.125F;
      float var3 = 0.5F;
      this.a(0.5F - var1, 0.5F - var2, 0.5F - var3, 0.5F + var1, 0.5F + var2, 0.5F + var3);
   }
}
